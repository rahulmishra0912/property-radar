...
export function SearchView() {
  ...
  useEffect(() => {
    ...
    fetch(withBasePath("/search-index.json"))
      ...
      .catch(() => {
        if (!cancelled) setIndex({ projects: [], developers: [] });
      });
    return () => {
      cancelled = true;
    };
  ...
  const { projects, developers }: { projects: SearchProjectHit[]; developers: SearchDeveloperHit[] } =
    index ? filterSearchIndex(index, q, 40) : { projects: [], developers: [] };
        ...
        <div className="mt-8 space-y-8">
          <section>
            ...
            <ul className="mt-3 space-y-3">
              {projects.map((p) => (
                <li key={p.id}>
                  <Link href={`/projects/${p.slug}`} className="report-card block rounded-2xl p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      <StatusBadge status={p.reraStatus} />
                      {p.delayMonths > 0 ? (
                        <span className="text-xs font-semibold text-alert">{p.delayMonths} mo delay</span>
                      ) : (
