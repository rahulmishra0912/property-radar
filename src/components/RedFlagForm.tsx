...
export function RedFlagForm({ projectId }: { projectId: string }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3 rounded-xl border border-alert/20 bg-alert-soft/40 p-4">
...
export function UpvoteButton({ id, initial }: { id: string; initial: number }) {
  ...
  return (
    <button
      type="button"
      onClick={vote}
      className="rounded-md border border-line bg-paper px-2 py-1 text-xs font-semibold text-navy hover:border-teal"
    >
      ▲ {n}
    </button>
  );
